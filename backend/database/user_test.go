...
func TestGetUsers(t *testing.T) {
	...
	prepareQuery := "SELECT * FROM User"

	t.Run("Success", func(t *testing.T) {
		mockStmt := mock.NewRows([]string{
			"id", "role_id", "login", "name", "baned", "created_at", "updated_at", "role_value", "role_translate_value",
		}).AddRow(
			1, 1, "admin", "Administrator", false, time.Now().Unix(), time.Now().Unix(), "admin", "Админ",
		).AddRow(
		...
		stmt, err := db.Prepare(prepareQuery)
		assert.NoError(t, err)
...
func TestUser_GetUser(t *testing.T) {
	...
	prepareQuery := "SELECT * FROM User WHERE id = $1"

	t.Run("Success", func(t *testing.T) {
		mock.ExpectPrepare(expectPrepareQuery).
			ExpectQuery().WithArgs(1).WillReturnRows(
			sqlmock.NewRows([]string{"role_id", "login", "name", "baned", "created_at", "updated_at", "role_value", "role_translate_value"}).
...
func TestUser_ChangeStatus(t *testing.T) {
	...
	prepareQuery := "UPDATE User SET baned = !baned WHERE id = $1"
	...
	t.Run("Success", func(t *testing.T) {
		...
		mock.ExpectPrepare(expectPrepareQuery).
			ExpectQuery().WithArgs(1).
			WillReturnRows(sqlmock.NewRows([]string{"baned"}).AddRow(true))

		stmt, err := db.Prepare(prepareQuery)
		assert.NoError(t, err)
		query["CHANGE_USER_STATUS"] = stmt

		err = user.ChangeStatus()
		assert.NoError(t, err)
		assert.Equal(t, true, user.Baned)
	})
...
func TestUser_EditUser(t *testing.T) {
	...
	prepareQuery := "UPDATE User SET role_id = $2, login = $3, name = $4, updated_at = $5 WHERE id = $1"
...
func TestUser_CreateUser(t *testing.T) {
	...
	prepareQuery := "INSERT INTO User (role_id, login, name, password, baned, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id"
...
func TestUser_GetAuthorize(t *testing.T) {
	...
	prepareQuery := "SELECT * FROM User WHERE login = $1 AND password = $2"
...
func TestDeleteSession(t *testing.T) {
	...
	prepareQuery := "DELETE FROM Session WHERE hash = $1"
...
func TestCreateSession(t *testing.T) {
	...
	prepareQuery := "INSERT INTO Session (hash, user_id, created_at) VALUES ($1, $2, $3)"
...
func TestLoadSession(t *testing.T) {
	...
	prepareQuery := "SELECT * FROM Session"
